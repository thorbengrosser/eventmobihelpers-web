...
def select_track():
    ...
    if form.validate_on_submit():
        ...
        sessions_to_delete = fetch_sessions_by_track(event_id, track_id_to_delete)
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [executor.submit(delete_session, event_id, session_item['id']) for session_item in sessions_to_delete]
...
def logout():
    from app.session import clear_session_data
    clear_session_data()
    return redirect(url_for('main.index'))
